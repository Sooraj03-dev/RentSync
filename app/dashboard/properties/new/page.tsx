...
import { Loader2, ArrowRight, ArrowLeft, UploadCloud, X, MapPin } from 'lucide-react';
...
export default function RegisterPropertyPage() {
  ...
  const handleNext = async () => {
    const fieldsByStep: any = {
      ...
      2: [],
      3: ['furnishing'],
      4: ['electricity_model', 'water_model'],
      5: []
    };
  ...
  const onSubmit = async (data: PropertyFormValues) => {
    ...
    try {
      ...
      router.push(`/dashboard/properties/${resData.id}`);
    } catch (e: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 pb-24">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">Register Property</h1>
        <p className="text-slate-500 text-sm mt-1">Complete your property profile to attract the best tenants.</p>
      </div>
                ...
                <div 
                  ...
                  onDrop={(e) => {
                    ...
                    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                      const newFiles = Array.from(e.dataTransfer.files).filter(f => f.type.startsWith('image/')).slice(0, 8 - photoFiles.length);
                      setPhotoFiles(prev => [...prev, ...newFiles]);
                    }
                  }}
                >
                  <UploadCloud className="w-10 h-10 mb-3 text-blue-400" />
                  <p className="text-sm font-bold text-slate-600">Click or Drag & Drop Photos</p>
                  <p className="text-xs mt-1 text-slate-400">Up to 8 images (JPG, PNG)</p>
                  <input 
                    type="file" 
                    id="photo-upload" 
                    multiple 
                    ...
                    onChange={(e) => {
                      if (e.target.files) {
                        const newFiles = Array.from(e.target.files).slice(0, 8 - photoFiles.length);
                        setPhotoFiles(prev => [...prev, ...newFiles]);
                      }
                    }}
                  />
